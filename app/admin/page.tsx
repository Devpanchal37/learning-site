  ...
  return (
    <div style={{ minHeight: "100vh", background: "#0D0D0F" }}>
      ...
      >
        ...
        >
          Complete a learning session with Claude, get the markdown file, and
          upload it here. The site will render all 11 sections automatically.
        </p>
        ...
        <StepCard number="01" title="Get the markdown template" color="#5B6AF0">
          ...
          >
            Ask Claude:{" "}
            <em style={{ color: "#7B8AF8" }}>
              "Generate the learning markdown file for [topic name]"
            </em>{" "}
            — Claude will fill the full 11-section format based on your session.
          </p>
        ...
        <StepCard number="02" title="Upload your markdown file" color="#10B981">
          ...
          {status === "success" && (
            ...
            >
              <div>
                ✓ Topic published to {publishMeta?.path || "content/topics"}
                {publishMeta?.branch ? ` (${publishMeta.branch})` : ""}.
                <Link
                  href={`/topic/${filename}`}
              ...
              >
                {publishMeta?.note ||
                  "If you are on a fixed preview URL, open the latest deployment or production domain after rebuild completes."}
              </div>
            </div>
          )}
